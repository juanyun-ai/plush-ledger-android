...
- Android App 是当前完整版本，当前源码与已验证发布包均为 **1.4.0**；GitHub Release、Cloudflare 官网和生产 `app_versions` 用户端更新记录已于 2026-07-12 验证生效。
- 微信小程序 **1.6.3** 已通过微信开发者工具上传为开发版，审核版本为空，尚未提审或正式发布；2026-07-12 通过微信公众平台复核的正式线上版为 **1.6.1**。由于个人主体小程序类目和隐私合规限制，小程序版收敛了生活日历、资料上传、手机绑定等入口，智能快记采用本机规则解析。
...
- GitHub Release：[v1.4.0](https://github.com/juanyun-ai/plush-ledger-android/releases/tag/v1.4.0)
...
- 绒绒日记支持登录后云端同步，同一账号在不同设备间可恢复历史日记。
...
- 生活日历、倒计时、礼物清单与心愿计划支持真实新增、编辑、删除和预算进度；生日资料可切换阳历/农历显示，并同步为首页日历标记。
- 账单详情支持直接编辑金额、分类、账户、日期和备注，不必删除后重新记账；本地模式也能通过 App 内在线留言提交反馈，不会上传账本。
...
> 生活日历、倒计时与心愿计划在 v1.0.1 按当前账户保存在本机，以避免在尚未发布专用云表和 RLS 策略前上传私人生活安排；已登录用户的账本、资料和头像仍按既有 Supabase 同步链路保存。
...
1. 界面字号改为“标准 / 紧凑 / 更紧凑”，移除放大选项，并限制系统字号继续放大 App 内布局，改善 vivo 等显示偏大机型的换行和挤压。
2. 记账足迹六个数据卡改为自适应横向信息布局，热力图保留但折叠到“记账活跃度”中，喜欢看的用户可展开查看。
3. 绒绒日记历史编辑暂存改为按单篇日记保存；近期日记勾选后在标题右侧显示取消和合并，不再弹出额外选择条。
4. 用户ID限制为 6-8 位英文字母并接入云端唯一性检查；地区选择补齐省级行政区及地市列表。
5. 主题色卡改为先选择预览、再确认切换；经典组恢复为原来的 8 种纯色。
...
1. 修复 1.1.5 更新包主下载地址实际返回官网 HTML，导致安装包完整性校验失败的问题。
2. App 内“浏览器下载”改为打开当前版本的真实 APK 下载线路，不再固定跳到旧下载页。
3. 增加视觉字号保护：系统字体或显示大小被放大时，App 内字号会收口到设计上限，减少 vivo 等机型出现挤压换行；默认字号的小米等机型布局不受影响。

### v1.1.5

1. 34 省级分享卡片重新同步为最新原图素材，并补齐重庆限定卡片；分享时不再叠加日期、心情或二维码遮罩。
2. 本地模式补充切换到邮箱登录前需自行 CSV 导出数据的提醒，避免用户误以为本地数据会自动迁移。
3. 管理后台连接失败提示改为可读原因，并重新部署 `admin-console` 后端函数。
...
6. 同版修订：主题选择改为 Compose 原生色彩、材质和状态绘制，移除截取设计图的方形贴片；新增生活日历、重要日子、倒计时、礼物与心愿的真实保存和编辑。
7. 同版修订：用户信息页补充农历生日展示、生日应用内提醒和首页标记；账单支持原地编辑；本地模式可通过默认邮箱给开发者发送建议。
