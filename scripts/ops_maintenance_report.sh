...
query_json() {
  ...
  rm -f "$temp"
  echo "Supabase 连续 8 次查询失败，本次巡检不生成过期报告。" >&2
  return 1
}

query_json "select public.admin_ops_stats() as stats;" "$TMP_STATS"
query_json "select platform, version_code, version_name, active, apk_url, backup_apk_url, file_size_bytes, updated_at from public.app_versions order by version_code desc limit 20;" "$TMP_VERSIONS"
...
DOCS_DOWNLOAD_BYTES="$(du -sk docs/downloads 2>/dev/null | awk '{print $1 * 1024}')"
...
BUILD_BYTES="$(du -sk build app/build 2>/dev/null | awk '{sum += $1} END {print sum * 1024}')"

node - "$TMP_STATS" "$TMP_VERSIONS" "$REPORT_FILE" "$DOCS_DOWNLOAD_COUNT" "$DOCS_DOWNLOAD_SIZE" "$DOCS_DOWNLOAD_BYTES" "$SHARE_CARD_COUNT" "$SHARE_CARD_SIZE" "$BUILD_SIZE" "$BUILD_BYTES" <<'NODE'
const fs = require("fs");
const [statsPath, versionsPath, outPath, docsCount, docsSize, docsBytes, shareCount, shareSize, buildSize, buildBytes] = process.argv.slice(2);
...
if (Number(docsCount) > 4 || Number(docsBytes) > 300 * 1024 * 1024) warnings.push("docs/downloads 已堆积较多旧 APK，清理前先确认旧版官方消息和回滚链接不再依赖它们。");
if (Number(buildBytes) > 1024 * 1024 * 1024) warnings.push("本地构建产物已超过 1GB，可在不需要调试时执行 Gradle clean 回收空间。");
const md = `# 绒绒记账月度运维巡检
